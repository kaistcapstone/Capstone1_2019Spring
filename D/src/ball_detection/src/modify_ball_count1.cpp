...
{
   ros::init(argc, argv, "modify_ball_count_node1"); //init ros nodd
   ros::NodeHandle nh; //create node handler
   pub = nh.advertise<core_msgs::ball_position>("/position_modify1", 100); //setting publisher
   ros::Subscriber sub1 = nh.subscribe<core_msgs::ball_position>("/position1", 1000, imageCallback);
