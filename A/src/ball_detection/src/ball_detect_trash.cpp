...
int** find_circle(Mat img)
{
  ...
  int real_ball = 0;
  int** circle_pix = int*[contours.size()];
  ...
  for( size_t i = 0; i< contours.size(); i++ )
  {
    ...
    {
      int pix_x=center[i].x;
      int pix_y=center[i].y;
      int radius_pix=((int)radius[i]-radcon);
      for (int j=0 ; j<real_ball ; j++)
      {
        if (carpos_pix[j][2]==-1)
        {
          continue;
        }
        int real_pix_x=carpos[j][0];
        int real_pix_y=carpos[j][1];
        int radius_sum=carpos[j][2]+radius_pix;
        int ball_dist_pix=pow(pix_x-real_pix_x,2)+pow(pix_y-real_pix_y,2);
        if(ball_dist_pix < pow(radius_sum,2)) {
          if (radius_pix>real_ball_radius_pix[j]) carpos_pix[j][2]=-1;
          else diff_ball=false;
        }
      }
      if (diff_ball)
      {
        radius[i]=radius[i]-radcon;
        carpos_pix[real_ball][0] = pix_x;
        carpos_pix[real_ball][1] = pix_y;
        carpos_pix[real_ball][2] = radius[i];
        real_ball++;
      }
    }
  }
  int small_ball_num=0;
  for (int i=0 ; i<real_ball;i++)
  {
    ...
    for (int j=0; j<3 ; j++)
    {
      carpos_pix[i-small_ball_num][j]=carpos_pix[i][j];
    }
  }
  real_ball=real_ball-small_ball_num;
...
float circle_depth(Mat depth, int**circle_pix, Scalar color)
{
  int real_ball=sizeof(circle_pix[0])/sizeof(circle_pix[0][0]);
  float* ball_mean_depth = new float[real_ball];
  for (int i=0 ; i<real_ball_r; i++)
  {
    Mat find_depth_img(Size(1920, 1080), CV_8UC3, Scalar(0,0,0));
    float pix_x_rat = real_ball_pix_x[i];
    float pix_y_rat = real_ball_pix_y[i];
...
float** circle_meter(int**circle_pix)
{
  int real_ball=sizeof(circle_pix[0])/sizeof(circle_pix[0][0]);
  float** carpos = new float*[real_ball];
  for(int j = 0; j < real_ball; ++j)
  {
    carpos_[j] = new float[3];
  }
  for (int i=0 ; i<real_ball ; i++)
  {
    Vector<Point2f> center = Point2f(circle_pix[i][0],circle_pix[i][1]);
    int radius = circle_pix[i][2];
    vector<float> ball_position;
    ball_position = pixel2point(center, radius);
...
void draw_circle(int** circle, Mat img)
{
  int real_ball=sizeof(circle[0])/sizeof(circle[0][0]);
  cout<<"real_ball: "<<real_ball<<endl;
...
*/
  ...
  for(int k=0; k<real_ball_r; k++) {
    if (ball_mean_depth_r[k] == 0) ball_mean_depth_r[k] = carpos_r[k][2];
  ...
  for(int k=0; k<real_ball_b; k++) {
    if (ball_mean_depth_b[k] == 0) ball_mean_depth_b[k] = carpos_b[k][2];
