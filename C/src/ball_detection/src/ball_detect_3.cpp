...
int low_h_r=0, low_s_r=206, low_v_r=92;
...
Mat distCoeffs;
float intrinsic_data[9] = {617.549485, 0, 327.528525, 0, 623.383275, 263.942651, 0, 0, 1};
float distortion_data[5] = {-0.010086, -0.080372, 0.011634, -0.004454, 0};
...
core_msgs::ball_position ball_detect(Mat frame){
    ...
    createTrackbar("Low H","Object Detection_HSV_Red_3", &low_h_r, 180, on_low_h_thresh_trackbar_red);
    createTrackbar("High H","Object Detection_HSV_Red_3", &high_h_r, 180, on_high_h_thresh_trackbar_red);
    createTrackbar("Low H2","Object Detection_HSV_Red_3", &low_h2_r, 180, on_low_h2_thresh_trackbar_red);
    createTrackbar("High H2","Object Detection_HSV_Red_3", &high_h2_r, 180, on_high_h2_thresh_trackbar_red);
    createTrackbar("Low S","Object Detection_HSV_Red_3", &low_s_r, 255, on_low_s_thresh_trackbar_red);
    createTrackbar("High S","Object Detection_HSV_Red_3", &high_s_r, 255, on_high_s_thresh_trackbar_red);
    createTrackbar("Low V","Object Detection_HSV_Red_3", &low_v_r, 255, on_low_v_thresh_trackbar_red);
    createTrackbar("High V","Object Detection_HSV_Red_3", &high_v_r, 255, on_high_v_thresh_trackbar_red);

    // Trackbars to set thresholds for HSV values : Blue ball
    createTrackbar("Low H","Object Detection_HSV_Blue_3", &low_h_b, 180, on_low_h_thresh_trackbar_blue);
    createTrackbar("High H","Object Detection_HSV_Blue_3", &high_h_b, 180, on_high_h_thresh_trackbar_blue);
    createTrackbar("Low S","Object Detection_HSV_Blue_3", &low_s_b, 255, on_low_s_thresh_trackbar_blue);
    createTrackbar("High S","Object Detection_HSV_Blue_3", &high_s_b, 255, on_high_s_thresh_trackbar_blue);
    createTrackbar("Low V","Object Detection_HSV_Blue_3", &low_v_b, 255, on_low_v_thresh_trackbar_blue);
    createTrackbar("High V","Object Detection_HSV_Blue_3", &high_v_b, 255, on_high_v_thresh_trackbar_blue);
	
	// Trackbars to set thresholds for HSV values : Green ball
    createTrackbar("Low H","Object Detection_HSV_Green_3", &low_h_g, 180, on_low_h_thresh_trackbar_green);
    createTrackbar("High H","Object Detection_HSV_Green_3", &high_h_g, 180, on_high_h_thresh_trackbar_green);
    createTrackbar("Low S","Object Detection_HSV_Green_3", &low_s_g, 255, on_low_s_thresh_trackbar_green);
    createTrackbar("High S","Object Detection_HSV_Green_3", &high_s_g, 255, on_high_s_thresh_trackbar_green);
    createTrackbar("Low V","Object Detection_HSV_Green_3", &low_v_g, 255, on_low_v_thresh_trackbar_green);
    createTrackbar("High V","Object Detection_HSV_Green_3", &high_v_g, 255, on_high_v_thresh_trackbar_green);


    // Trackbar to set parameter for Canny Edge
    createTrackbar("Min Threshold:","Canny Edge for Red Ball_3", &lowThreshold_r, 100, on_canny_edge_trackbar_red);
    createTrackbar("Min Threshold:","Canny Edge for Blue Ball_3", &lowThreshold_b, 100, on_canny_edge_trackbar_blue);
	createTrackbar("Min Threshold:","Canny Edge for Green Ball_3", &lowThreshold_g, 100, on_canny_edge_trackbar_green);
   ...
   // while((char)waitKey(1)!='q'){
       ...
       // cout << contours_r.size() << endl << "----------" << endl;
		...
		float cam_height = 0.34, y_offset = 0; // y_offset is distance between cam and center of robot
        ...
        for( size_t i = 0; i< contours_b.size(); i++ ){
            if(radius_b[i] > iMin_tracking_ball_size){
				//cout << i << "// center : " << center_b[i].x << ", " << center_b[i].y << "// radius : " << radius_b[i] << endl;               
		...
		for( size_t i = 0; i< contours_g.size(); i++ ){
            if(radius_g[i] > iMin_tracking_ball_size){
				...
				//ROS_INFO("green - x : %f, y : %f",out.img_x_green[j], out.img_y_green[j]);
				j += 1;
        ...
        // Show the frames
/*        imshow("Video Capture_3",calibrated_frame);
        imshow("Object Detection_HSV_Red_3",hsv_frame_red);
        imshow("Object Detection_HSV_Blue_3",hsv_frame_blue);
		imshow("Object Detection_HSV_Green_3",hsv_frame_green);
        imshow("Canny Edge for Red Ball_3", hsv_frame_red_canny);
        imshow("Canny Edge for Blue Ball_3", hsv_frame_blue_canny);
		imshow("Canny Edge for Green Ball_3", hsv_frame_green_canny);*/
        imshow("Result_3", result);
   // }
...
class pass{
	...
	public :
		pass(){
			...
			pub = nh.advertise<core_msgs::ball_position>("/position_3", 100);
			//pub_markers = nh.advertise<visualization_msgs::Marker>("/balls",1);
			sub = it.subscribe("camera/image_3", 1, &pass::imageCallback, this);
...
int main(int argc, char **argv){
	...
	ros::init(argc, argv, "ball_detect_3");

	pass PASS;

/*	namedWindow("Video Capture_3", WINDOW_NORMAL);
    namedWindow("Object Detection_HSV_Red_3", WINDOW_NORMAL);
    namedWindow("Object Detection_HSV_Blue_3", WINDOW_NORMAL);
	namedWindow("Object Detection_HSV_Green_3", WINDOW_NORMAL);
    namedWindow("Canny Edge for Red Ball_3", WINDOW_NORMAL);
    namedWindow("Canny Edge for Blue Ball_3", WINDOW_NORMAL);
	namedWindow("Canny Edge for Green Ball_3", WINDOW_NORMAL);*/
    namedWindow("Result_3", WINDOW_NORMAL);
/*    moveWindow("Video Capture_3", 1310, 0);
    moveWindow("Object Detection_HSV_Red_3", 1310,370);
    moveWindow("Object Detection_HSV_Blue_3",1730,370);
	moveWindow("Object Detection_HSV_Green_3",2150,370);
    moveWindow("Canny Edge for Red Ball_3", 1310,730);
    moveWindow("Canny Edge for Blue Ball_3", 1730,730);
	moveWindow("Canny Edge for Green Ball_3", 2150,730);*/
    moveWindow("Result_3", 50, 730);
...
void on_low_h_thresh_trackbar_red(int, void *){
    ...
    setTrackbarPos("Low H","Object Detection_HSV_Red_3", low_h_r);
}

void on_high_h_thresh_trackbar_red(int, void *){
    high_h_r = max(high_h_r, low_h_r+1);
    setTrackbarPos("High H", "Object Detection_HSV_Red_3", high_h_r);
}

void on_low_h2_thresh_trackbar_red(int, void *){
    low_h2_r = min(high_h2_r-1, low_h2_r);
    setTrackbarPos("Low H2","Object Detection_HSV_Red_3", low_h2_r);
}
...
void on_low_s_thresh_trackbar_red(int, void *){
    ...
    setTrackbarPos("Low S","Object Detection_HSV_Red_3", low_s_r);
}

void on_high_s_thresh_trackbar_red(int, void *){
    high_s_r = max(high_s_r, low_s_r+1);
    setTrackbarPos("High S", "Object Detection_HSV_Red_3", high_s_r);
}

void on_low_v_thresh_trackbar_red(int, void *){
    low_v_r= min(high_v_r-1, low_v_r);
    setTrackbarPos("Low V","Object Detection_HSV_Red_3", low_v_r);
}

void on_high_v_thresh_trackbar_red(int, void *){
    high_v_r = max(high_v_r, low_v_r+1);
    setTrackbarPos("High V", "Object Detection_HSV_Red_3", high_v_r);
}

// Trackbar for image threshodling in HSV colorspace : Blue
void on_low_h_thresh_trackbar_blue(int, void *){
    low_h_b = min(high_h_b-1, low_h_b);
    setTrackbarPos("Low H","Object Detection_HSV_Blue_3", low_h_b);
}

void on_high_h_thresh_trackbar_blue(int, void *){
    high_h_b = max(high_h_b, low_h_b+1);
    setTrackbarPos("High H", "Object Detection_HSV_Blue_3", high_h_b);
}

void on_low_s_thresh_trackbar_blue(int, void *){
    low_s_b = min(high_s_b-1, low_s_b);
    setTrackbarPos("Low S","Object Detection_HSV_Blue_3", low_s_b);
}

void on_high_s_thresh_trackbar_blue(int, void *){
    high_s_b = max(high_s_b, low_s_b+1);
    setTrackbarPos("High S", "Object Detection_HSV_Blue_3", high_s_b);
}

void on_low_v_thresh_trackbar_blue(int, void *){
    low_v_b= min(high_v_b-1, low_v_b);
    setTrackbarPos("Low V","Object Detection_HSV_Blue_3", low_v_b);
}

void on_high_v_thresh_trackbar_blue(int, void *){
    high_v_b = max(high_v_b, low_v_b+1);
    setTrackbarPos("High V", "Object Detection_HSV_Blue_3", high_v_b);
}

// Trackbar for image threshodling in HSV colorspace : Green
void on_low_h_thresh_trackbar_green(int, void *){
    low_h_g = min(high_h_g-1, low_h_g);
    setTrackbarPos("Low H","Object Detection_HSV_Green_3", low_h_g);
}

void on_high_h_thresh_trackbar_green(int, void *){
    high_h_g = max(high_h_g, low_h_g+1);
    setTrackbarPos("High H", "Object Detection_HSV_Green_3", high_h_g);
}

void on_low_s_thresh_trackbar_green(int, void *){
    low_s_g = min(high_s_g-1, low_s_g);
    setTrackbarPos("Low S","Object Detection_HSV_Green_3", low_s_g);
}

void on_high_s_thresh_trackbar_green(int, void *){
    high_s_g = max(high_s_g, low_s_g+1);
    setTrackbarPos("High S", "Object Detection_HSV_Green_3", high_s_g);
}

void on_low_v_thresh_trackbar_green(int, void *){
    low_v_g= min(high_v_g-1, low_v_g);
    setTrackbarPos("Low V","Object Detection_HSV_Green_3", low_v_g);
}

void on_high_v_thresh_trackbar_green(int, void *){
    high_v_g = max(high_v_g, low_v_g+1);
    setTrackbarPos("High V", "Object Detection_HSV_Green_3", high_v_g);
}


// Trackbar for Canny edge algorithm
void on_canny_edge_trackbar_red(int, void *){
    setTrackbarPos("Min Threshold", "Canny Edge for Red Ball_3", lowThreshold_r);
}

void on_canny_edge_trackbar_blue(int, void *){
    setTrackbarPos("Min Threshold", "Canny Edge for Blue Ball_3", lowThreshold_b);
}

void on_canny_edge_trackbar_green(int, void *){
    setTrackbarPos("Min Threshold", "Canny Edge for Green Ball_3", lowThreshold_g);
}
