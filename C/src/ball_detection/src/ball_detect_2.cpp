...
int low_h_r=0, low_s_r=153, low_v_r=84;
...
int high_h_b=110, high_s_b=255, high_v_b=240;
...
int low_h_g=51, low_s_g=60, low_v_g=94;
int high_h_g=84, high_s_g=245, high_v_g=253;
...
Mat distCoeffs;
float intrinsic_data[9] = {642.684732, 0, 339.646184, 0, 648.308290, 264.631910, 0, 0, 1};
float distortion_data[5] = {0.040375, -0.095099, 0.015688, 0.003085, 0};
...
core_msgs::ball_position ball_detect(Mat frame){
    ...
    createTrackbar("Low H","Object Detection_HSV_Red_2", &low_h_r, 180, on_low_h_thresh_trackbar_red);
    createTrackbar("High H","Object Detection_HSV_Red_2", &high_h_r, 180, on_high_h_thresh_trackbar_red);
    createTrackbar("Low H2","Object Detection_HSV_Red_2", &low_h2_r, 180, on_low_h2_thresh_trackbar_red);
    createTrackbar("High H2","Object Detection_HSV_Red_2", &high_h2_r, 180, on_high_h2_thresh_trackbar_red);
    createTrackbar("Low S","Object Detection_HSV_Red_2", &low_s_r, 255, on_low_s_thresh_trackbar_red);
    createTrackbar("High S","Object Detection_HSV_Red_2", &high_s_r, 255, on_high_s_thresh_trackbar_red);
    createTrackbar("Low V","Object Detection_HSV_Red_2", &low_v_r, 255, on_low_v_thresh_trackbar_red);
    createTrackbar("High V","Object Detection_HSV_Red_2", &high_v_r, 255, on_high_v_thresh_trackbar_red);

    // Trackbars to set thresholds for HSV values : Blue ball
    createTrackbar("Low H","Object Detection_HSV_Blue_2", &low_h_b, 180, on_low_h_thresh_trackbar_blue);
    createTrackbar("High H","Object Detection_HSV_Blue_2", &high_h_b, 180, on_high_h_thresh_trackbar_blue);
    createTrackbar("Low S","Object Detection_HSV_Blue_2", &low_s_b, 255, on_low_s_thresh_trackbar_blue);
    createTrackbar("High S","Object Detection_HSV_Blue_2", &high_s_b, 255, on_high_s_thresh_trackbar_blue);
    createTrackbar("Low V","Object Detection_HSV_Blue_2", &low_v_b, 255, on_low_v_thresh_trackbar_blue);
    createTrackbar("High V","Object Detection_HSV_Blue_2", &high_v_b, 255, on_high_v_thresh_trackbar_blue);
	
	// Trackbars to set thresholds for HSV values : Green ball
    createTrackbar("Low H","Object Detection_HSV_Green_2", &low_h_g, 180, on_low_h_thresh_trackbar_green);
    createTrackbar("High H","Object Detection_HSV_Green_2", &high_h_g, 180, on_high_h_thresh_trackbar_green);
    createTrackbar("Low S","Object Detection_HSV_Green_2", &low_s_g, 255, on_low_s_thresh_trackbar_green);
    createTrackbar("High S","Object Detection_HSV_Green_2", &high_s_g, 255, on_high_s_thresh_trackbar_green);
    createTrackbar("Low V","Object Detection_HSV_Green_2", &low_v_g, 255, on_low_v_thresh_trackbar_green);
    createTrackbar("High V","Object Detection_HSV_Green_2", &high_v_g, 255, on_high_v_thresh_trackbar_green);


    // Trackbar to set parameter for Canny Edge
    createTrackbar("Min Threshold:","Canny Edge for Red Ball_2", &lowThreshold_r, 100, on_canny_edge_trackbar_red);
    createTrackbar("Min Threshold:","Canny Edge for Blue Ball_2", &lowThreshold_b, 100, on_canny_edge_trackbar_blue);
	createTrackbar("Min Threshold:","Canny Edge for Green Ball_2", &lowThreshold_g, 100, on_canny_edge_trackbar_green);
   ...
   // while((char)waitKey(1)!='q'){
        ...
        Canny(eq_red, hsv_frame_red_canny, lowThreshold_r, lowThreshold_r*ratio_r, kernel_size_r);
        Canny(eq_blue, hsv_frame_blue_canny, lowThreshold_b, lowThreshold_b*ratio_b, kernel_size_b);
		Canny(eq_green, hsv_frame_green_canny, lowThreshold_g, lowThreshold_g*ratio_g, kernel_size_g);
/*
		Mat hist1 = hsv_frame_green_blur, hist2 = eq_green;
		resize(hist1, hist1, Size(), 0.2, 0.2, CV_INTER_AREA);
		resize(hist2, hist2, Size(), 0.2, 0.2, CV_INTER_AREA);
		...
		calcHist(&hist1, 1, channel_numbers, Mat(), histogram1, 1, &number_bins, &channel_ranges);
		calcHist(&hist2, 1, channel_numbers, Mat(), histogram2, 1, &number_bins, &channel_ranges);

		int hist_w = 1000; int hist_h = 400;
		int bin_w = cvRound((double)hist_w/number_bins);

		Mat histImage1(hist_h, hist_w, CV_8UC1, Scalar(0,0,0));
		normalize(histogram1, histogram1, 0, histImage1.rows, NORM_MINMAX, -1, Mat());
		...
		for (int i = 1;i<number_bins;i++){
			line(histImage1, Point(bin_w*(i-1), hist_h - cvRound(histogram1.at<float>(i-1))), Point(bin_w*(i), hist_h - cvRound(histogram1.at<float>(i))), Scalar(255,0,0),2,8,0);
			line(histImage2, Point(bin_w*(i-1), hist_h - cvRound(histogram2.at<float>(i-1))), Point(bin_w*(i), hist_h - cvRound(histogram2.at<float>(i))), Scalar(255,0,0),2,8,0);
		}

		
		imshow("before histogram",histImage1);
		imshow("after histogram",histImage2);
...
*/
        ...
        for( size_t i = 0; i < contours_r.size(); i++ ){
            approxPolyDP( contours_r[i], contours_r_poly[i], 2, true );
       ...
       // cout << contours_r.size() << endl << "----------" << endl;
        ...
        for( size_t i = 0; i < contours_b.size(); i++ ){
            approxPolyDP( contours_b[i], contours_b_poly[i], 2, true );
		...
		for( size_t i = 0; i < contours_g.size(); i++ ){
            approxPolyDP( contours_g[i], contours_g_poly[i], 2, true );
		...
		float cam_height = 0.37, y_offset = 0; //y_offset : distance between cam and center of robot
		...
		for(size_t i = 0; i<contours_g.size();i++){
			if(radius_g[i] > iMin_tracking_ball_size) green_cnt += 1;
		}
        ...
        for( size_t i = 0; i< contours_r.size(); i++ ){
            if (radius_r[i] > iMin_tracking_ball_size){
              ...
              //  cout << i << "// center : " << center_r[i].x << ", " << center_r[i].y << "// radius : " << radius_r[i] << endl;
        ...
        for( size_t i = 0; i< contours_b.size(); i++ ){
            if(radius_b[i] > iMin_tracking_ball_size){
				...
				//ROS_INFO("BLUE BALL, center : (%f, %f), radius : %f", center_b[i].x, center_b[i].y, radius_b[i]);
        ...
        // Show the frames
    /*    imshow("Video Capture_2",calibrated_frame);
        	
        
        
		imshow("before equalize red", hsv_frame_red_blur);
		imshow("before equalize blue", hsv_frame_blue_blur);
		imshow("before equalize green", hsv_frame_green_blur);
		imshow("after equalize red", eq_red);
		imshow("after equalize blue", eq_blue);
		imshow("after equalize green", eq_green);

*
		imshow("Object Detection_HSV_Red_2",hsv_frame_red);
		imshow("Object Detection_HSV_Blue_2",hsv_frame_blue);
		imshow("Object Detection_HSV_Green_2",hsv_frame_green);
		imshow("Canny Edge for Red Ball_2", hsv_frame_red_canny);
        imshow("Canny Edge for Blue Ball_2", hsv_frame_blue_canny);
		imshow("Canny Edge for Green Ball_2", hsv_frame_green_canny);*/
        imshow("Result_2", result);
...
class pass{
	...
	public :
		pass(){
			...
			pub = nh.advertise<core_msgs::ball_position>("/position_2", 100);
			//pub_markers = nh.advertise<visualization_msgs::Marker>("/balls",1);
			sub = it.subscribe("camera/image_2", 1, &pass::imageCallback, this);
...
int main(int argc, char **argv){
	...
	ros::init(argc, argv, "ball_detect_2");

	pass PASS;

/*	namedWindow("Video Capture_2", WINDOW_NORMAL);
    

	namedWindow("before equalize red", WINDOW_NORMAL);
	namedWindow("before equalize blue", WINDOW_NORMAL);
	namedWindow("before equalize green", WINDOW_NORMAL);
	namedWindow("after equalize red", WINDOW_NORMAL);
	namedWindow("after equalize blue", WINDOW_NORMAL);
	namedWindow("after equalize green", WINDOW_NORMAL);
	namedWindow("before histogram", CV_WINDOW_AUTOSIZE);
	namedWindow("after histogram", CV_WINDOW_AUTOSIZE);

	namedWindow("Object Detection_HSV_Red_2", WINDOW_NORMAL);
    namedWindow("Object Detection_HSV_Blue_2", WINDOW_NORMAL);
	namedWindow("Object Detection_HSV_Green_2", WINDOW_NORMAL);	
	namedWindow("Canny Edge for Red Ball_2", WINDOW_NORMAL);
    namedWindow("Canny Edge for Blue Ball_2", WINDOW_NORMAL);
	namedWindow("Canny Edge for Green Ball_2", WINDOW_NORMAL);*/
	namedWindow("Result_2", WINDOW_NORMAL);
/*    moveWindow("Video Capture_2", 1310, 0);
    moveWindow("Object Detection_HSV_Red_2", 50,370);
    moveWindow("Object Detection_HSV_Blue_2",420,370);
	moveWindow("Object Detection_HSV_Green_2",870,370);
	moveWindow("Canny Edge for Red Ball_2", 50,370);
    moveWindow("Canny Edge for Blue Ball_2", 420,370);
	moveWindow("Canny Edge for Green Ball_2", 870,370);    
	
    */
	moveWindow("Result_2", 50, 370);
...
void on_low_h_thresh_trackbar_red(int, void *){
    ...
    setTrackbarPos("Low H","Object Detection_HSV_Red_2", low_h_r);
}

void on_high_h_thresh_trackbar_red(int, void *){
    high_h_r = max(high_h_r, low_h_r+1);
    setTrackbarPos("High H", "Object Detection_HSV_Red_2", high_h_r);
}

void on_low_h2_thresh_trackbar_red(int, void *){
    low_h2_r = min(high_h2_r-1, low_h2_r);
    setTrackbarPos("Low H2","Object Detection_HSV_Red_2", low_h2_r);
}
...
void on_low_s_thresh_trackbar_red(int, void *){
    ...
    setTrackbarPos("Low S","Object Detection_HSV_Red_2", low_s_r);
}

void on_high_s_thresh_trackbar_red(int, void *){
    high_s_r = max(high_s_r, low_s_r+1);
    setTrackbarPos("High S", "Object Detection_HSV_Red_2", high_s_r);
}

void on_low_v_thresh_trackbar_red(int, void *){
    low_v_r= min(high_v_r-1, low_v_r);
    setTrackbarPos("Low V","Object Detection_HSV_Red_2", low_v_r);
}

void on_high_v_thresh_trackbar_red(int, void *){
    high_v_r = max(high_v_r, low_v_r+1);
    setTrackbarPos("High V", "Object Detection_HSV_Red_2", high_v_r);
}

// Trackbar for image threshodling in HSV colorspace : Blue
void on_low_h_thresh_trackbar_blue(int, void *){
    low_h_b = min(high_h_b-1, low_h_b);
    setTrackbarPos("Low H","Object Detection_HSV_Blue_2", low_h_b);
}

void on_high_h_thresh_trackbar_blue(int, void *){
    high_h_b = max(high_h_b, low_h_b+1);
    setTrackbarPos("High H", "Object Detection_HSV_Blue_2", high_h_b);
}

void on_low_s_thresh_trackbar_blue(int, void *){
    low_s_b = min(high_s_b-1, low_s_b);
    setTrackbarPos("Low S","Object Detection_HSV_Blue_2", low_s_b);
}

void on_high_s_thresh_trackbar_blue(int, void *){
    high_s_b = max(high_s_b, low_s_b+1);
    setTrackbarPos("High S", "Object Detection_HSV_Blue_2", high_s_b);
}

void on_low_v_thresh_trackbar_blue(int, void *){
    low_v_b= min(high_v_b-1, low_v_b);
    setTrackbarPos("Low V","Object Detection_HSV_Blue_2", low_v_b);
}

void on_high_v_thresh_trackbar_blue(int, void *){
    high_v_b = max(high_v_b, low_v_b+1);
    setTrackbarPos("High V", "Object Detection_HSV_Blue_2", high_v_b);
}

// Trackbar for image threshodling in HSV colorspace : Green
void on_low_h_thresh_trackbar_green(int, void *){
    low_h_g = min(high_h_g-1, low_h_g);
    setTrackbarPos("Low H","Object Detection_HSV_Green_2", low_h_g);
}

void on_high_h_thresh_trackbar_green(int, void *){
    high_h_g = max(high_h_g, low_h_g+1);
    setTrackbarPos("High H", "Object Detection_HSV_Green_2", high_h_g);
}

void on_low_s_thresh_trackbar_green(int, void *){
    low_s_g = min(high_s_g-1, low_s_g);
    setTrackbarPos("Low S","Object Detection_HSV_Green_2", low_s_g);
}

void on_high_s_thresh_trackbar_green(int, void *){
    high_s_g = max(high_s_g, low_s_g+1);
    setTrackbarPos("High S", "Object Detection_HSV_Green_2", high_s_g);
}

void on_low_v_thresh_trackbar_green(int, void *){
    low_v_g= min(high_v_g-1, low_v_g);
    setTrackbarPos("Low V","Object Detection_HSV_Green_2", low_v_g);
}

void on_high_v_thresh_trackbar_green(int, void *){
    high_v_g = max(high_v_g, low_v_g+1);
    setTrackbarPos("High V", "Object Detection_HSV_Green_2", high_v_g);
}


// Trackbar for Canny edge algorithm
void on_canny_edge_trackbar_red(int, void *){
    setTrackbarPos("Min Threshold", "Canny Edge for Red Ball_2", lowThreshold_r);
}

void on_canny_edge_trackbar_blue(int, void *){
    setTrackbarPos("Min Threshold", "Canny Edge for Blue Ball_2", lowThreshold_b);
}

void on_canny_edge_trackbar_green(int, void *){
    setTrackbarPos("Min Threshold", "Canny Edge for Green Ball_2", lowThreshold_g);
}
